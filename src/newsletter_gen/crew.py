...
class NewsletterGen():
	...
	def llm(self):
		...
		# llm = ChatGroq(model="groq/deepseek-r1-distill-llama-70b")
	...
	def researcher(self) -> Agent:
		return Agent(
			...
			tools=[SearchAndContents(), FindSimilar(), GetContents()],# Add tools here
			verbose=True,
			llm=self.llm(),
	...
	def designer(self) -> Agent:
		return Agent(
			...
			llm=self.llm(),
			step_callback=lambda step: self.step_callback(step, "Design Agent")
		)
